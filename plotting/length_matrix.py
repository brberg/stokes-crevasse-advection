...
if __name__ == "__main__":
    ...
    directories = ['baseline_noad','warm_noad','slippery_noad', 'both_noad']
    ...
    for j in range(len(axA)):
        plot_length_gl(directories[j], time_start, time, axA[j], bed_interpolator, 3, 1, 'No Advection w/ Grounded Cliff', 'No Advection w/ Ice Tongue')#, ls1='--')
    ...
    ax4.set(ylabel=r"$\Delta$L (m)")

    ax4.set(xlabel=r"Time (a)")

    xlimsA = [axA[j].get_xlim() for j in range(4)]
    ylimsA = [axA[j].get_ylim() for j in range(4)]
    ...
    directories = ['baseline','warm','slippery', 'both']

    plot_names = ['baseline', 'warm ice', 'slippery bed', 'both']
    ...
    for j in range(len(axB)):
        plot_length_gl(directories[j], time_start, time, axB[j], bed_interpolator, 3, 1, 'No Advection w/ Grounded Cliff', 'No Advection w/ Ice Tongue')
        os.chdir(main_directory)
    ...
    ax8.set(xlabel=r"Time (a)")

    xlimsB = [axB[j].get_xlim() for j in range(4)]
    ylimsB = [axB[j].get_ylim() for j in range(4)]
    ylimsmin = [min(ylimsA[j][0], ylimsB[j][0]) for j in range(4)]
    ylimsmax = [max(ylimsA[j][1], ylimsB[j][1]) for j in range(4)]

    ymintop = min(ylimsmin[0], ylimsmin[1])
    ymaxtop = max(ylimsmax[0], ylimsmax[1])
    yminbot = min(ylimsmin[2], ylimsmin[3])
    ymaxbot = max(ylimsmax[2], ylimsmax[3])

    ylimsminfinal = [ymintop, ymintop, yminbot, yminbot]
    ylimsmaxfinal = [ymaxtop, ymaxtop, ymaxbot, ymaxbot]
    ...
    for axis in [ax1, ax2, ax3, ax5, ax6, ax7]:
        plt.sca(axis)
