...
def makeImages(iter, snapshots, geometryX, geometryY, data_names, folder_name, time):
    ...
    for n in range(len(snapshots)):
        ...
        geometryX = np.linspace(0, 60000, 10000)
    ...
    fig.savefig(folder_name + "/" + "animation_bump" + str(iter) + ".png", transparent=False, dpi=300)
    plt.close(fig)
...
if __name__ == "__main__":
    ...
    max_time = 50.0
    time_step = 1/12
    directories = ['baseline_bump_noad', 'baseline_bump']
    dataName = 'width_timeseries'
    folder_name = "animation_bump"
    ...
    for i in range(len(times)):
        imraw = mpimg.imread("animation_bump" + str(i) + ".png")
    ...
    ani.save("animation_bump.mp4", writer=writer, dpi=300) #dpi=500
